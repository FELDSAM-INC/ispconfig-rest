...
| `api/components/schemas/AccountBackupOverview.yaml` | one website entry: `web_domain_id`, `domain`, `server_id`, `backups_available`, `total`, `latest[]` → the shipped `WebBackup` schema |
...
- **Field parity is structural, not documented by hand**: `latest[]` references the shipped `WebBackup`
  schema itself, so the overview cannot drift from the per-website list.
