...
- [x] T001 [P] Create `api/components/schemas/MailPasswordPolicy.yaml`, `MailConnection.yaml`, `AccountMailServer.yaml`, `AccountMailSettings.yaml`; add required `mail` to `AccountCapabilities.yaml` and the four counts to `UsageSummary.yaml`; register the new schemas in `api/components/schemas/_index.yaml`
- [x] T002 Create `api/modules/me/mail-settings.yaml`, register it in `api/modules/me/_index.yaml` and `api/openapi.yaml`; extend the example/description of `api/modules/me/capabilities.yaml` and `api/modules/usage/summary.yaml`; document the tab gates and the `custom_mailfilter` refusal in `api/modules/mail/user-autoresponder.yaml`, `user-spamfilter.yaml`, `user-filters.yaml`
...
- [x] T004 Add `SystemConfigService::rawConfig()` and `rawSection(string $section)` (stored values, `[]` without table or row) in `app/Services/SystemConfigService.php`
- [x] T005 Create `app/Services/AccountMailService.php` with `tabAllowed(AuthScope, string $setting)`, `passwordPolicy()`, `accountMailServers(int $clientId)`, `dkimPathUsable(int $serverId)` (research R2, R5, R6)
...
- [x] T012 [US4] Extend `tests/Feature/UsageSummaryApiTest.php` (16 counts; catch-alls, alias domains, filters, fetchmail used/limit, unlimited → null)
- [x] T013 [US4] Add the four keys to `ClientLimitService::USAGE_COUNT_COLUMNS` and `countSpecForColumn()` in `app/Services/ClientLimitService.php`
