...
try {
    ...
    while (microtime(true) - $started < 50) {
        if (time() - $heartbeat >= 5) {
            $db->prepare('INSERT INTO api_web_log_workers (server_id, heartbeat) VALUES (?, ?) ON DUPLICATE KEY UPDATE heartbeat = VALUES(heartbeat)')->execute([$server, time()]);
        ...
        foreach ($query->fetchAll() as $job) {
            try {
                $website = $db->prepare("SELECT domain FROM web_domain WHERE domain_id = ? AND server_id = ? AND sys_groupid = ? AND domain = ? AND type IN ('vhost','vhostsubdomain','vhostalias')");
                $website->execute([$job['website_id'], $server, $job['sys_groupid'], $job['domain']]);
                $domain = $website->fetchColumn();
                if (! is_string($domain)) {
                    throw new RuntimeException('logs_unavailable');
                }
                $request = json_decode($job['request'], true, 8, JSON_THROW_ON_ERROR);
                $result = $reader->read($domain, $request['kind'], $request['lines'], $request['before']);
            } catch (Throwable $e) {
                $result = ['error' => in_array($e->getMessage(), ['logs_archive_limit', 'logs_archive_invalid', 'logs_rotated'], true) ? $e->getMessage() : 'logs_unavailable'];
            }
