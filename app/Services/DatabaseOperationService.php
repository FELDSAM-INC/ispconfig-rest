...
class DatabaseOperationService
{
    ...
    public function create(WebDatabase $source, array $input): array
    {
        ...
        return DB::transaction(function () use ($source, $input, $dump): array {
            ...
            $busy = DB::table('api_database_operations')->whereIn('status', ['queued', 'running'])
            ...
            $row = [
                ...
                'database_name' => $source->database_name_full, 'action' => $input['action'], 'status' => 'queued',
    ...
    public function find(WebDatabase $source, string $id): object
    {
        ...
        if (in_array($row->status, ['queued', 'running'], true) && $row->updated_at < time() - 1800) {
            DB::table('api_database_operations')->where('id', $id)->where('status', $row->status)
