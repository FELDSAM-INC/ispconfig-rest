...
The capabilities further carry `sites`: the name prefixes new databases, database users, FTP, SSH and WebDAV accounts of the account receive (already resolved, so a panel can show the full name before the first entry exists), the plan's database quota cap and remote-access permission, whether SSH access is available, with which isolation modes and which credential the installation accepts for SSH accounts (`sites.shell.authentication`: `password_or_key`, `password` or `key`), and which kinds of scheduled task the account may create with which shortest interval. Those task rules are enforced: for client and reseller keys a schedule running more often than `limit_cron_frequency` is refused with 403 `limit-reached`, and a command that is not a URL is refused with 403 `feature-not-allowed` (`limit_cron_type`) when the plan allows URL tasks only. `GET /api/v1/usage/summary` counts database users against `limit_database_user`, the cap that endpoint already enforced.
...
`GET /api/v1/me/hosting-links` returns the two tool addresses a panel offers next to databases and FTP accounts: the database administration address per database server of the account (the installation's `phpmyadmin_url` with `[SERVERNAME]` resolved — a `[DATABASENAME]` placeholder is left for the consumer to replace with the database's own prefixed name), and the web file manager address verbatim (`webftp_url`). Each part carries `available`, which follows the interface's own switch (`dblist_phpmyadmin_link`) and whether an address is configured, so a panel hides a button instead of showing a dead link. No other system configuration becomes readable for scoped keys.

DNS zones and records follow the panel's own rules (spec 033): the dynamic update ACL (`update_acl`) can only be set with an administrator key, and renaming a zone (`origin`) needs an administrator or reseller key — a scoped key that changes either gets `422` with `error_types.<field>` = `feature-not-allowed`, while re-sending the stored value is accepted (`xfer`, `also_notify` and the DNSSEC fields stay writable for every key). For every key type the API also refuses, as the ISPConfig forms do, an identical MX, TLSA or DKIM record in the zone and a second SPF record for the same name.

`GET /api/v1/dns/zone-templates` lists the zone templates the DNS wizard offers: every key sees the templates marked visible, whoever owns them, exactly as the ISPConfig wizard does, while `/api/v1/dns/templates` keeps its row-level scoping and administrator-only writes. `POST /api/v1/dns/soa/from-template` expands one of them into a complete zone — placeholders replaced, the zone written inactive, every template record created, then the zone activated — in one transaction and one `X-Change-Set-Id`. Every placeholder the template declares must be supplied (legacy writes a literal `{IP}` into the zone when one is missing), and the account's DNS zone and DNS record limits are both enforced for the whole batch before anything is written, unlike the ISPConfig wizard, which checks only the zone limit. `dkim` adds the published DKIM record of the domain's mail domain when the key can read one, and `dnssec` creates the zone with signing requested.

`GET /api/v1/dns/soa/{id}/dnssec` reports whether a zone is signed and returns the DS record its registrar needs — parsed out of the notes ISPConfig's DNS server writes — together with the published DNSKEY records. `state` is `off`, `pending`, `signed`, or `unavailable` when the zone's DNS server is mirrored: ISPConfig signs on the master only, so the panel hides DNSSEC there and the API refuses to switch it on, with `422` and `error_types.dnssec_wanted` = `feature-not-allowed` (switching it off, and re-sending the stored value, stay accepted). Signing is still switched with `PUT /api/v1/dns/soa/{id}` and `dnssec_wanted`; switching it off keeps the keys ISPConfig generated, so the DS record stays valid if it is switched back on. The raw `dnssec_info` column is returned to administrator keys only — on PowerDNS installations it carries a log of the commands the server ran, so client and reseller keys get `null` and read the parsed records instead.
...
| `usage` | read-only usage statistics: plan summary against limits, website/mailbox/database usage, traffic history, and collector freshness (how often each figure is collected, when it last was, when the next run is due) — every key |
...
- **Backups can be downloaded over HTTP** — an addition, not a parity gap: ISPConfig has no HTTP download at all, it only copies an archive into the website's `backup` folder for FTP/SSH. `GET`/`HEAD /sites/web-domains/{id}/backups/{backup_id}/download` streams that copy. It works because ISPConfig adds the web server user to every client group (how Apache serves the 0750 client directories), so the copy is readable by the API; the archives under the server's `backup_dir` stay root-only and are never touched. Where that does not hold — hardened permissions, a different runtime user, or a backup stored on another server — the endpoint answers `409 download-not-readable` and the backup reports `download.http: false`. The API never changes permissions to make itself able to read, and it refuses any path that leaves the website's backup folder.
