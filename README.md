...
- **Backups can be downloaded over HTTP where the operator allows it** — an addition, not a parity gap: ISPConfig has no HTTP download at all, it only copies an archive into the website's `backup` folder for FTP/SSH. `GET`/`HEAD /sites/web-domains/{id}/backups/{backup_id}/download` streams that copy, but only when the API process can read it. On a stock installation it cannot (the copy is `<system_user>:<system_group>` mode 0640 while the API runs as the web server user), so the endpoint answers `409 download-not-readable` and every backup reports `download.http: false`. The API never changes those permissions to make itself able to read; granting it access (for example a read ACL for the API user on website backup folders) is an operator decision, and it widens what the web server user can read — weigh that before enabling it.
